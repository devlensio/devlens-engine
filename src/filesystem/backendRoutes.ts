...
import fs from "fs";
import type { BackendRouteNode, BackendFramework, Framework } from "../types.js";
...
function readStringProperty(obj: any, propNames: string[]): string | null {
...
function hasRouteTableShape(obj: any): boolean {
  const methodRaw = readStringProperty(obj, METHOD_PROP_NAMES);
  if (methodRaw === null) return false;
  if (!HTTP_METHODS_SET.has(methodRaw.toLowerCase())) return false;
  const pathRaw = readStringProperty(obj, PATH_PROP_NAMES);
  if (pathRaw === null || !pathRaw.startsWith("/")) return false;
  return true;
}
...
function resolveHandlerLiteral(init: any): {
  handlerName?: string;
  inlineHandler?: { rawCode: string; startLine: number; endLine: number };
} {
  if (!init) return {};
...
function detectRouteTableRoutes(
  file: any,
  filePath: string,
  framework: BackendFramework,
  out: BackendRouteNode[],
  seen: Set<string>,
): void {
  for (const arr of file.getDescendantsOfKind(SyntaxKind.ArrayLiteralExpression)) {
    ...
    if (elements[0].getKind() !== SyntaxKind.ObjectLiteralExpression) continue;
    if (!hasRouteTableShape(elements[0])) continue;

    for (const el of elements) {
      if (el.getKind() !== SyntaxKind.ObjectLiteralExpression) continue;
      if (!hasRouteTableShape(el)) continue;
...
function isUrlReference(node: any): boolean {
...
function detectBunServeRoutes(
  ...
  for (const call of file.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    if (call.getExpression().getText() !== "Bun.serve") continue;

    const args = call.getArguments();
    if (args.length === 0) continue;
    const cfg = args[0];
    if (cfg.getKind() !== SyntaxKind.ObjectLiteralExpression) continue;
    ...
    if (!fetchBody || !fetchBody.getDescendantsOfKind) continue;
    ...
    for (const bin of fetchBody.getDescendantsOfKind(SyntaxKind.BinaryExpression)) {
      ...
      if (!isUrlReference(otherSide)) continue;
...
function includesImport(fileContent: string, mod: string): boolean {
  return (
...
function literalStringValue(node: any): string | null {
  if (!node) return null;
...
function detectFileFramework(
  ...
  if (includesImport(fileContent, "elysia")) return "elysia";
  if (includesImport(fileContent, "hono")) return "hono";
  if (includesImport(fileContent, "express")) return "express";
  if (includesImport(fileContent, "fastify")) return "fastify";
  if (includesImport(fileContent, "koa")) return "koa";

  if (fileContent.includes("Bun.serve(") || includesImport(fileContent, "bun")) return "bun";  // Bun is a runtime, not a framework, so we check for Bun.serve() usage or bun import.
...
export function analyzeBackendRoutes(repoPath: string, framework: Framework): BackendRouteNode[] {
  ...
  const fileFrameWorks = new Map<string, BackendFramework>();
  ...
  for (const file of project.getSourceFiles()) {
    ...
    const framework =
      fileFrameWorks.get(filePath) ?? detectFileFramework(content) ?? "bun";
    ...
    for (const call of callExpressions) {
      try {
        ...
        if (seen.has(_imperativeKey)) continue;
