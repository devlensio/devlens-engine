...
function extractComponentName(valueNode: any): string | undefined {
  if (!valueNode) return undefined;
  const k = valueNode.getKind();
  if (k === SyntaxKind.JsxSelfClosingElement) return valueNode.getTagNameNode().getText();
  if (k === SyntaxKind.JsxElement) return valueNode.getOpeningElement().getTagNameNode().getText();
  if (k === SyntaxKind.Identifier) return valueNode.getText();
  return undefined;
}

// Unwraps a JSX attribute initializer (element={<Home/>}) to its inner expression.
function unwrapJsxAttr(attr: any): any {
  if (!attr || !attr.getInitializer) return null;
...
function objComponentName(obj: any): string | undefined {
...
function findReactFiles(dir: string, files: string[] = []): string[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (/\.(tsx|jsx|ts|js)$/.test(entry.name)) files.push(fullPath);
    }
  }
  return files;
}

export function analyzeReactRouterRoutes(repoPath: string): RouteNode[] {
  ...
  for (const file of project.getSourceFiles()) {
    ...
    for (const el of [
      ...
      const pathAttr = el.getAttribute("path") as any;
      if (!pathAttr || !pathAttr.getInitializer) continue;

      const init = pathAttr.getInitializer();
      if (!init) continue;

      let valueNode: any = init;
      if (init.getKind() === SyntaxKind.JsxExpression) {
        valueNode = (init as any).getExpression();
      }
      const raw = literalString(valueNode);
      if (raw === null) continue;       // computed path — skip

      // element={<Home/>} (v6) or component={Home} / Component={Home} (v5)
      const rendersComponent =
        extractComponentName(unwrapJsxAttr(el.getAttribute("element"))) ??
    ...
    for (const call of file.getDescendantsOfKind(SyntaxKind.CallExpression)) {
      ...
      if (calleeLeaf === "createFileRoute") {
        ...
        if (args.length > 0) pushRoute(nodes, literalString(args[0]), filePath, seen);
        continue;
      }
      ...
      if (calleeLeaf === "createRoute") {
        ...
        if (args.length > 0 && args[0].getKind() === SyntaxKind.ObjectLiteralExpression) {
          const pathProp = (args[0] as any).getProperty("path");
          if (pathProp && pathProp.getInitializer) {
