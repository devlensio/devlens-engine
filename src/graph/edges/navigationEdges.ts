...
interface NavRouteIndex {
  staticByPath: Map<string, NavRouteEntry>;
  dynamic:      NavRouteEntry[];
}

// Build the route index from navigable ROUTE nodes (NOT API routes).
function buildNavRouteIndex(nodes: CodeNode[]): NavRouteIndex {
  const staticByPath = new Map<string, NavRouteEntry>();
  const dynamic: NavRouteEntry[] = [];
  ...
  for (const routeNode of routeNodes) {
    ...
    const entry: NavRouteEntry = { routeNode, urlPath, isDynamic, urlRegex: urlPathtoRegex(urlPath) };
    if (isDynamic) dynamic.push(entry);
    else if (!staticByPath.has(urlPath)) staticByPath.set(urlPath, entry);
  }
  return { staticByPath, dynamic };
}

// Two-pass match: static exact first, then dynamic regex.
function matchNavRoute(normalizedUrl: string, index: NavRouteIndex): NavRouteEntry | null {
  const exact = index.staticByPath.get(normalizedUrl);
  if (exact) return exact;
  for (const e of index.dynamic) {
    if (e.urlRegex.test(normalizedUrl)) return e;
  }
  return null;
}

// Per-file map: local imported name → module specifier.
function buildImportSourceMap(sourceFile: SourceFile): Map<string, string> {
...
function buildNavBindingMaps(sourceFile: SourceFile, importMap: Map<string, string>) {
  const navigateVars = new Set<string>();   // useNavigate (RR / TanStack)
  const routerVars   = new Set<string>();   // useRouter   (Next.js)
  const historyVars  = new Set<string>();   // useHistory  (RR v5)
  ...
  for (const decl of sourceFile.getDescendantsOfKind(SyntaxKind.VariableDeclaration)) {
    ...
    if (nameNode.getKind() !== SyntaxKind.Identifier) continue;   // skip destructuring
    const name = nameNode.getText();

    if (callee === "useNavigate" && RR_LIBS.includes(src)) navigateVars.add(name);
    else if (callee === "useRouter" && NEXT_ROUTER_LIBS.includes(src)) routerVars.add(name);
    else if (callee === "useHistory" && RR_LIBS.includes(src)) historyVars.add(name);
  }
  return { navigateVars, routerVars, historyVars };
}

function getJsxAttrValueNode(attr: any): any {
  const init = attr.getInitializer?.();
  if (!init) return null;
  const k = init.getKind();
  if (k === SyntaxKind.StringLiteral) return init;
  if (k === SyntaxKind.JsxExpression) return init.getExpression();
  return null;
}

interface LocatedNav {
  startLine:   number;
  callerType:  string;
  rawUrl:      string;
  resolvedUrl: string;
  method:      string;   // navigate | push | replace | location | history | link
}

function extractNavCallsFromFile(sourceFile: SourceFile, project: Project): LocatedNav[] {
  const results: LocatedNav[] = [];
  ...
  for (const call of sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    if (expr.getKind() === SyntaxKind.Identifier) {
      ...
      if (!navigateVars.has(name) || args.length === 0) continue;

      const arg0 = args[0];
      let urlResult = null;
      if (arg0.getKind() === SyntaxKind.ObjectLiteralExpression) {
        const toInit = (arg0 as any).getProperty("to")?.getInitializer?.();
    ...
    if (expr.getKind() === SyntaxKind.PropertyAccessExpression) {
      ...
      if (propName === "push" || propName === "replace") {
        ...
        if (!eligible || args.length === 0) continue;

        const arg0 = args[0];
        let urlResult = null;
        if (arg0.getKind() === SyntaxKind.ObjectLiteralExpression) {
          const pInit = (arg0 as any).getProperty("pathname")?.getInitializer?.();
  ...
  for (const bin of sourceFile.getDescendantsOfKind(SyntaxKind.BinaryExpression)) {
    ...
    const member = left as any;
    if (member.getName() !== "href") continue;
    if (!member.getExpression().getText().endsWith("location")) continue;
  ...
  for (const el of [
    ...
    if (tag !== "Link" && tag !== "NavLink") continue;

    const importSrc = importMap.get(tag);
    if (!importSrc) continue;                          // gate: must be an imported Link

    const toAttr   = el.getAttribute("to") as any;
    const hrefAttr = el.getAttribute("href") as any;

    let attr: any = null;
    if (toAttr && RR_LIBS.includes(importSrc)) attr = toAttr;
    else if (hrefAttr && NEXT_LINK_LIBS.includes(importSrc)) attr = hrefAttr;
    if (!attr) continue;
...
function makeGhostRoute(id: string, target: string, flags: Record<string, unknown>): CodeNode {
  return {
    id,
    name: target,
    type: "ROUTE",
...
export function detectNavigationEdges(
  nodes: CodeNode[],
  repoPath: string,
): { edges: CodeEdge[]; ghostNodes: CodeNode[] } {
  const edges: CodeEdge[] = [];
  const dedupSet = new Set<string>();
  const ghostMap = new Map<string, CodeNode>();
  ...
  const candidateNodes = nodes.filter(n =>
    n.type !== "ROUTE" && n.type !== "THIRD_PARTY" &&
  ...
  for (const node of candidateNodes) {
    ...
    if (!nodesByFile.has(abs)) nodesByFile.set(abs, []);
  ...
  for (const [abs, fileNodes] of nodesByFile) {
    ...
    if (!sourceFile) {
      try { sourceFile = project.addSourceFileAtPath(abs); } catch { continue; }
    }
    ...
    for (const loc of located) {
      ...
      for (const node of fileNodes) {
        if (loc.startLine < node.startLine || loc.startLine > node.endLine) continue;
      ...
      if (rawTarget.startsWith("http://") || rawTarget.startsWith("https://")) {
        ...
        if (!normalized) continue;   // relative / unparseable → can't reason, skip
