...
export function detectInheritanceEdges(nodes: CodeNode[], lookupMp: LookupMaps): CodeEdge[] {
  const edges: CodeEdge[] = [];
  const created = new Set<string>();

  const resolveTarget = (typeName: string, fromFile: string): CodeNode | undefined => {
  ...
  for (const node of nodes) {
    ...
    // ── EXTENDS ─────────────────────────────────────────────────────────
    const extendsType = node.metadata.extendsType as string | undefined;
    if (extendsType) {
    ...
    const implementsTypes = node.metadata.implementsTypes as string[] | undefined;
    if (implementsTypes && implementsTypes.length > 0) {
      for (const impl of implementsTypes) {
        ...
        if (!target || target.type !== "CLASS" || target.id === node.id) continue;
