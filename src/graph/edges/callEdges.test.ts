...
function detectCalls(repoPath: string, extraNodes: ReturnType<typeof buildThirdPartyNodes> = []) {
...
describe("detectCallEdges with class methods", () => {
  ...
  it("does not resolve instance calls through local variables", () => {
    ...
    expect((caller?.metadata.calls as string[])).toContain("svc.get");
    deleteFakeRepo(repo);
  });
  ...
  it("writes resolvedCalls back onto METHOD nodes", () => {
    ...
    expect(bump?.metadata.resolvedCalls).toEqual(
      expect.arrayContaining([expect.objectContaining({ name: "Counter.inc", nodeId: "src/wb.ts::Counter.inc" })]),
    );
    deleteFakeRepo(repo);
  });
});
