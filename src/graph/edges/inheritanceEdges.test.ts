...
function detect(repoPath: string) {
...
describe("detectInheritanceEdges", () => {
  ...
  it("emits EXTENDS for a same-file class-to-class inheritance", () => {
    ...
    expect(edges).toContainEqual(
      expect.objectContaining({ from: "src/inh.ts::Child", to: "src/inh.ts::Base", type: "EXTENDS" }),
    );
  ...
  it("emits EXTENDS across files via name lookup", () => {
    ...
    const edges = detect(repo);
    expect(edges).toContainEqual(
      expect.objectContaining({ from: "src/child.ts::Child", to: "src/base.ts::Base", type: "EXTENDS" }),
    );
    deleteFakeRepo(repo);
  });
  ...
  it("strips generics from extends before resolving", () => {
    ...
    const edges = detect(repo);
    expect(edges).toContainEqual(
      expect.objectContaining({ from: "src/gen.ts::Child", to: "src/gen.ts::Base", type: "EXTENDS" }),
    );
  ...
  it("emits IMPLEMENTS only for local class targets, ignores interfaces", () => {
    ...
    expect(implementsEdges[0]).toEqual(
      expect.objectContaining({ from: "src/impl.ts::Worker", to: "src/impl.ts::BaseClass" }),
    );
    deleteFakeRepo(repo);
  });
