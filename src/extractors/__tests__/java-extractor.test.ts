...
function runJava(fixture: string, options: Record<string, unknown> = {}) {
  const res = spawnSync(
    "java", ["-jar", JAR],
    {
...
const memo = new Map<string, ReturnType<typeof runJava>>();
function analyze(fixture: string, options: Record<string, unknown> = {}) {
  const key = fixture + "|" + JSON.stringify(options);
  if (!memo.has(key)) memo.set(key, runJava(fixture, options));
  return memo.get(key)!;
}
...
describe.skipIf(!jarAvailable || !javaOnPath)("java extractor (contract + fixtures)", () => {
  ...
  describe("springboot fixture", () => {
    ...
    test("node type census", () => {
      const byType = d.nodes.reduce((acc: Record<string, number>, n: any) => {
    ...
    test("routes: class+method path composition, no double slashes", () => {
      ...
      for (const r of d.routes) expect(r.urlPath).not.toContain("//");
    });
    ...
    test("HANDLES: every route → its controller method, 0 broken", () => {
      ...
      for (const h of handles) expect(ids.has(h.to), `handler exists: ${h.to}`).toBe(true);
      const byPath = new Map(handles.map((h: any) => [`${h.metadata.httpMethod} ${h.metadata.urlPath}`, h.to]));
    ...
    test("CALLS: controller → service (4) and service → repository (4)", () => {
      ...
      expect(calls.some((e: any) =>
        e.from.endsWith("UserController.listUsers") && e.to === svc + ".getAllUsers")).toBe(true);
      expect(calls.some((e: any) =>
        e.from.endsWith("UserController.createUser") && e.to === svc + ".createUser")).toBe(true);
      ...
      expect(calls.filter((e: any) => e.to === repo)).toHaveLength(4);
      // symbol-solver resolved call to the base class
      expect(calls.some((e: any) => e.to.endsWith("BaseService.logAction"))).toBe(true);
    });
    ...
    test("JPA model: User isModel=jpa with fields[]", () => {
      ...
      const names = user.metadata.fields.map((f: any) => f.name);
    ...
    test("R/W edges: service reads+writes User, repo reads User (consumer→store)", () => {
      const rw = d.edges
        ...
        .map((e: any) => `${e.from.split("::").pop()} ${e.type} ${e.to.split("::").pop()}`)
        .sort();
      expect(rw).toEqual([
    ...
    test("gating: allowed libs → [mvn] nodes + IMPORTS + gated EXTENDS", () => {
      ...
      const tp = nodesOf(g, (n) => n.type === "THIRD_PARTY").map((n: any) => n.id);
      expect(tp).toContain("[mvn]/org.springframework");
      expect(tp).toContain("[mvn]/jakarta.persistence::Entity");
      expect(edgesOf(g, "IMPORTS").filter((e: any) => e.to.startsWith("[mvn]")).length).toBeGreaterThan(0);
      expect(edgesOf(g, "EXTENDS").some((e: any) => e.to === "[mvn]/org.springframework::JpaRepository")).toBe(true);
    });
  ...
  describe("plain fixture (no framework)", () => {
    ...
    test("records parse: Shape CLASS kind=record, isSchema, component fields", () => {
      ...
      expect(shape.metadata.fields.map((f: any) => f.name)).toEqual(["width", "height"]);
    ...
    test("inner class → dotted id Outer.Inner; call resolves into it", () => {
      ...
      const calls = edgesOf(d, "CALLS");
      expect(calls.some((e: any) => e.to.endsWith("Outer.Inner.doubleValue"))).toBe(true);
    });

    test("interface default method call resolves within the interface", () => {
      const calls = edgesOf(d, "CALLS");
      expect(calls.some((e: any) =>
        e.from.endsWith("Greeter.greetLoud") && e.to.endsWith("Greeter.greet"))).toBe(true);
    });
    ...
    test("no third-party without gating; JDK imports skipped entirely", () => {
      ...
      expect(imports.filter((e: any) => e.to.includes("java/util"))).toHaveLength(0);
    });
  ...
  describe("gradle fixture", () => {
    ...
    test("route + call chain", () => {
      expect(d.routes.map((r: any) => `${r.httpMethod} ${r.urlPath}`)).toEqual(["GET /greetings"]);
      const calls = edgesOf(d, "CALLS");
  ...
  describe("weird fixture (annotation edge cases)", () => {
    ...
    test("path params → isDynamic + params (braces stripped)", () => {
      const del = d.routes.find((r: any) => r.httpMethod === "DELETE");
