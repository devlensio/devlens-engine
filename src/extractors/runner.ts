...
export async function runSubprocessExtractor(extractor: LanguageExtractor, input: ExtractorInput, timeoutMs: number = 10 * 60 * 1000): Promise<ExtractorResult> {
    return new Promise((resolve, reject) => {
        ...
        if (jarArg && !fs.existsSync(jarArg)) {
            reject(new Error(
                `${extractor.language} extractor artifact not found: ${jarArg}. ` +
        ...
        if (extractor.language === "java" && !commandExists("java")) {
            reject(new Error(
                "java extractor requires a Java 17+ runtime (JVM) on PATH — " +
        ...
        if (extractor.language === "python" && !commandExists(extractor.command)) {
            reject(new Error(
                "python extractor unavailable: no Python 3.11+ found. Install Python, " +
        ...
        if (extractor.language === "go" && !fs.existsSync(extractor.command)) {
            reject(new Error(
                `go extractor binary not found: ${extractor.command}. ` +
        ...
        const timer = setTimeout(() => {
            child.kill("SIGTERM");
            reject(new Error(
                `${extractor.language} extractor timed out after ${timeoutMs}ms`
            ));
        }, timeoutMs);
        ...
        child.on("close", (code) => {
            ...
            if (code !== 0) {
                ...
                reject(new Error(
                    `${extractor.language} extractor exited with code ${code}.\n${stderr}`
                ));
                return;
            }
            try {
                ...
                reject(new Error(
                    `Failed to parse ${extractor.language} extractor output: ` +
...
export async function runInlineExtractor(input: ExtractorInput, onStep?: (step: "fingerprint" | "filesystem" | "parse" | "edges" | "scoring") => void): Promise<ExtractorResult> {
    ...
    const thirdPartyNodes: CodeNode[] = buildThirdPartyNodes(absoluteRepoPath, options.includeThirdPartyLibs || []);
    ...
    routeNodes = routeNodes.filter(routeNode => {
        if (routeNode.metadata.routeNodeType === "API_ROUTE") {
            return edgeResult.edges.some(
                (edge) => edge.type === "HANDLES" && edge.from === routeNode.id
            );
        }
        return true;
    });
...
export async function runExtractor(
    ...
    if (!extractor) {
    throw new Error(
      `No extractor registered for language: "${language}". ` +
