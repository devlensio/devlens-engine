...
const VALID_PROTOCOLS = new Set(["openai", "anthropic"] as const);
const RESERVED_NAMES = new Set(["__custom__"]);

function validateProvider(entry: CatalogProviderInput): void {
  const { name, label, protocol, baseUrl, requiresKey } = entry;

  if (typeof name !== "string" || !/^[a-z0-9-]+$/.test(name) || name.length === 0) {
    throw new Error(
      `Invalid provider name: "${name}". Must be non-empty, lowercase alphanumeric with hyphens.`
    );
  }
  ...
  if (!VALID_PROTOCOLS.has(protocol as any)) {
...
function readUserFile(): CatalogProvider[] {
  if (!fs.existsSync(USER_CATALOG_FILE)) return [];
  try {
    ...
    console.warn(
      `DevLens: ~/.devlens/providers.json contains invalid JSON. ` +
...
export function loadCatalog(): CatalogProvider[] {
  const defaults = DEFAULT_PROVIDERS;
  const userProviders = readUserFile();

  if (userProviders.length === 0) return defaults;
...
export function findProvider(name: string): CatalogProvider | undefined {
  return loadCatalog().find(p => p.name === name);
}

/** Returns only the user's overrides (not merged with defaults). */
export function listUserProviders(): CatalogProvider[] {
  return readUserFile();
}
 ...
 */
export function saveProvider(entry: CatalogProviderInput): CatalogProvider {
  validateProvider(entry);

  const existing = readUserFile();
  const idx = existing.findIndex(p => p.name === entry.name);
  const clean: CatalogProvider = { name: entry.name, label: entry.label, protocol: entry.protocol, baseUrl: entry.baseUrl, requiresKey: entry.requiresKey };
