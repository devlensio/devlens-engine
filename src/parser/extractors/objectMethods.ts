...
function asObjectLiteral(node: any): any | null {
  let n = node;
  while (
    n &&
...
function isAsyncFn(fnNode: any): boolean {
  if (typeof fnNode.isAsync === "function") return fnNode.isAsync();
...
function walkObject(
  ...
  for (const prop of objLiteral.getProperties()) {
    ...
    if (kind !== SyntaxKind.PropertyAssignment) continue;  // skip spread/shorthand/get/set
    const name = typeof prop.getName?.() !== "string" ? undefined : prop.getName().replace(/^['"`]|['"`]$/g, "");
...
export function extractObjectMethods(
  file: SourceFile,
  fileDirective: RenderingBoundary = null,
): CodeNode[] {
  const out: CodeNode[] = [];
  ...
  for (const assign of file.getExportAssignments()) {
    if (assign.isExportEquals?.()) continue;  // skip `export = ...`
