...
export function stripGenerics(typeText: string): string {
...
function isReactBase(extendsType: string): boolean {
...
function parseComponentGenerics(extendsType: string): { propsType?: string; stateType?: string } {
  ...
  const out: { propsType?: string; stateType?: string } = { propsType: args[0] };
  if (args.length > 1) out.stateType = args[1];
  return out;
}
...
// node name.
function rewriteThisCalls(calls: string[], className: string, methodNames: Set<string>): string[] {
  return calls.map((call) => {
    if (!call.startsWith("this.")) return call;
    const rest = call.slice("this.".length);
    const firstSegment = rest.split(".")[0].replace(/^#/, "");
    if (!methodNames.has(firstSegment)) return call;
...
function isPrivateMethod(method: any, name: string): boolean {
  if (name.startsWith("#")) return true;
  return method.getModifiers().some((m: any) => m.getKind() === SyntaxKind.PrivateKeyword);
}
...
function buildMethodNode(
  ...
  fileDirective: RenderingBoundary,
): CodeNode {
  ...
  return {
    ...
    metadata: {
      ...
      isAccessor: undefined as "get" | "set" | undefined,
...
function collectMethodNames(cls: any): string[] {
  const names: string[] = [];
  for (const m of cls.getMethods?.() ?? []) {
    const n = m.getName?.();
    if (typeof n === "string") names.push(n.startsWith("#") ? n.slice(1) : n);
  }
  for (const c of cls.getConstructors?.() ?? []) names.push("constructor");
...
export function extractClasses(file: SourceFile, fileDirective: RenderingBoundary = null): CodeNode[] {
  ...
  for (const cls of file.getClasses()) {
    ...
    const implementsTypes = cls.getImplements().map((t: any) => t.getExpression().getText());
    ...
    const constructorParams = constructors[0]
      ? constructors[0].getParameters().map((p: any) => p.getName())
      : [];

    const isExported = typeof (cls as any).isExported === "function" ? (cls as any).isExported() : false;
    ...
    for (const m of cls.getMethods()) {
      ...
      if (typeof methodName !== "string" || methodName === "") continue; // computed names
      const isHashPrivate = methodName.startsWith("#");
      if (isHashPrivate) methodName = methodName.slice(1);
    ...
    for (const acc of cls.getGetAccessors()) {
      ...
      if (typeof accName !== "string") continue;
