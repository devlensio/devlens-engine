...
export const PRUNABLE_TYPES = new Set<string>([
  "STRUCT",
  "INTERFACE",
  "ENUM",
  "PACKAGE",
  ...
  for (const node of kept) {
    if (node.type === "FILE") {
      ...
      if (Array.isArray(childIds)) {
        const clean = childIds.filter((id) => !removed.has(id as string));
    ...
    if (Array.isArray(resolvedCalls)) {
      const cleanCalls = resolvedCalls.filter(
        (r) => !removed.has((r as { nodeId?: string }).nodeId ?? "")
      );
