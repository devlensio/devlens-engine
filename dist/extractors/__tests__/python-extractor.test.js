...
const VENV_PYTHON = path.resolve(import.meta.dir, "../../../extractors/python/.venv/bin/python");
const FIXTURES = path.resolve(import.meta.dir, "../../../extractors/python/tests/fixtures");
const pythonAvailable = fs.existsSync(VENV_PYTHON);
// ─── Helpers ────────────────────────────────────────────────────────────
function runPython(fixture, options = {}) {
    const res = spawnSync(VENV_PYTHON, ["-m", "devlens_extractors_python"], {
...
function runPythonRepo(repoPath, options = {}) {
    const res = spawnSync(VENV_PYTHON, ["-m", "devlens_extractors_python"], {
...
function analyze(fixture, options = {}) {
    ...
    if (!memo.has(key))
        memo.set(key, runPython(fixture, options));
    return memo.get(key);
}
...
describe.skipIf(!pythonAvailable)("python extractor (contract + fixtures)", () => {
    ...
    describe("contract shape", () => {
        ...
        test("empty repo → valid empty result", () => {
            expect(Object.keys(empty).sort()).toEqual(["edges", "errors", "fingerprint", "nodes", "routes", "stats"].sort());
        ...
        test("fingerprint always carries the full key set", () => {
            expect(Object.keys(empty.fingerprint).sort()).toEqual(["databases", "dataFetching", "framework", "language", "projectType",
    ...
    describe("pyfixture (fastapi)", () => {
        ...
        test("route GET /users/{user_id} with HANDLES → get_user", () => {
            expect(d.routes.map((r) => `${r.httpMethod} ${r.urlPath}`)).toEqual([
                "GET /users/{user_id}",
            ]);
    ...
    describe("djfixture (django)", () => {
        ...
        test("celery tasks marked isTask", () => {
            const tasks = nodesOf(d, (n) => n.metadata.isTask).map((n) => n.id);
    ...
    describe("drfx (DRF routers)", () => {
        ...
        test("HANDLES → viewset CLASSES (8 handlers, 2 unique targets)", () => {
            const targets = [...new Set(edgesOf(d, "HANDLES").map((e) => e.to))].sort();
        ...
        test("models detected when django is gated", () => {
            ...
            const models = nodesOf(g, (n) => n.metadata.isModel).map((n) => n.id).sort();
    ...
    describe("ormfix (sqlalchemy data layer)", () => {
        ...
        test("models: Base/User/Post → sqlalchemy", () => {
            const models = nodesOf(d, (n) => n.metadata.isModel).map((n) => ({
        ...
        test("pydantic schemas → isSchema", () => {
            expect(nodesOf(d, (n) => n.metadata.isSchema).map((n) => n.id).sort()).toEqual([
        ...
        test("routes + call chain intact under gating", () => {
            ...
            expect(calls.some((e) => e.to === "[pip]/sqlalchemy::select")).toBe(true);
        });
    });
    // ── third-party gating ───────────────────────────────────────────────
    describe("third-party gating", () => {
        ...
        test("includeThirdPartyLibs (engine spelling) → [pip] nodes + IMPORTS", () => {
            ...
            const tp = nodesOf(d, (n) => n.type === "THIRD_PARTY").map((n) => n.id);
            expect(tp).toContain("[pip]/flask");
            // named from-import → edge to the MEMBER node
            const imports = edgesOf(d, "IMPORTS").filter((e) => e.to.startsWith("[pip]/flask"));
    ...
    describe("robustness", () => {
        test("syntax-error file → errors[] entry, other files still analyzed, exit 0", () => {
            ...
            const res = spawnSync(VENV_PYTHON, ["-m", "devlens_extractors_python"], { input: JSON.stringify({ repoPath: repo, options: {} }), encoding: "utf8" });
